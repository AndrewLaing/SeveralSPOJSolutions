...
MMDS has done its job and your job is to find the sum of hotness bonds 
...
The second line contains N integers separated by single spaces denoting 
the hotness levels of the men.
The third line contains N integers separated by single spaces denoting 
...
int calculateScore(const int *arrMen, const int *arrFem, int arrLen)
{
	int score = 0;
	int i;
	for(i=0; i<arrLen; i++)
...
int main()
{
	...
	int arrFem[] = {3, 2};
	cout << "Hotness score = " 
		 << calculateScore( arrMen,	 arrFem,  arrLen )
		 << endl; 
	...
	int arrFem1[] = {1, 3, 2};
	cout << "Hotness score = " 
		 << calculateScore( arrMen1,  arrFem1,	arrLen1 )
		 << endl; 
